...
```{r set-parent, echo=FALSE,warning=FALSE,error=FALSE,message=FALSE,cache=TRUE,warning=FALSE,include=FALSE,comment=NA}
   ...
   if(FALSE) 
      rmarkdown::render("/Users/user/dmc2015/features/feature_files/R/feature_matrix_version0.8.rmd")
```
...
makeWideFeatureMatrix = function(w,uf,set){
   ...
   saveRDS(Fmat1,file=paste0("~/dmc2015/data/featureMatrix/featMat_based-on-HTV",set,"_WIDE_ver0.8.rds"))
}
...
makeLongFeatureMatrix = function(l,u,set){
   ...
   names(Xcls) = gsub("\\.x","",names(Xcls))
   cls = list("X" = Xcls %>% select(-couponUsed,-basketValue),
              "y" = Xcls %>% select(orderID,couponUsed, basketValue))

   Fmat1 = list("train" = trn, "class" = cls, "validation" = val)
   saveRDS(Fmat1,file=paste0("~/dmc2015/data/featureMatrix/featMat_based-on-HTV",set,"_LONG_ver0.8.rds"))
}
```
...
```{r cache=FALSE}
   ...
   #weichang's features
   wc1 = readRDS("../set1/individual/weichangsFeature.rds") %>% select(-userID)
   ...
   #weichang's features
   wc3 = readRDS("../set3/individual/weichangsFeature.rds") %>% select(-userID) 
