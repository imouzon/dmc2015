---
title: Creating Feature Matrix Version 0.5
titleshort: Feature Matrix v. 0.5
instructions: 
output:
  usefulR::DMC_format
---
...
```{r cache=FALSE}
   ...
   list.files("~/dmc2015/features/feature_files/universal/individual/",full.name=TRUE)
...
```{r cache=FALSE}
   ...
   names(coupon3) = gsub("_13$","_to1Col",names(coupon3))
   names(coupon3) = gsub("_3$","",names(coupon3))
   coupon3$couponCol = 3

   coupons = bind_rows(coupon1,coupon2,coupon3)  %>% arrange(orderID,couponCol)
   for(i in grep("_to2Col",names(coupons))) coupons[is.na(coupons[,i]) & coupons$couponCol == 2,i] = 1
   for(i in grep("_to1Col",names(coupons))) coupons[is.na(coupons[,i]) & coupons$couponCol == 1,i] = 1
   for(i in grep("_to3Col",names(coupons))) coupons[is.na(coupons[,i]) & coupons$couponCol == 3,i] = 1
      
   names(coupons)[grep("_to\\dCol",names(coupons))]
   coupons$sim_ftd_invmax_cosSim_mean = apply(coupons[,grep("sim_ftd_invmax",names(coupons))],1,mean)
   coupons$sim_ftd_ivf_cosSim_mean = apply(coupons[,grep("sim_ftd_ivf",names(coupons))],1,mean)
   coupons$sim_ftd_lnorm_cosSim_mean = apply(coupons[,grep("sim_ftd_lnorm",names(coupons))],1,mean)
   coupons$sim_ftd_pif_cosSim_mean = apply(coupons[,grep("sim_ftd_pif",names(coupons))],1,mean)
   coupons$sim_jaccard_mean = apply(coupons[,grep("jaccard",names(coupons))],1,mean)
...
<!--- : R code (No Results in Document) -->
```{r cache=FALSE}
   wideFrame[,"sim_ftd_ivf_mean"] =  rowMeans(wideFrame[,grep("sim_ftd_ivf",names(wideFrame))])
   wideFrame[,"sim_ftd_lnorm_mean"] =  rowMeans(wideFrame[,grep("sim_ftd_lnorm",names(wideFrame))])
   wideFrame[,"sim_ftd_pif_mean"] =  rowMeans(wideFrame[,grep("sim_ftd_pif",names(wideFrame))])
   wideFrame[,"sim_ftd_invmax_mean"] =  rowMeans(wideFrame[,grep("sim_ftd_invmax",names(wideFrame))])

   names(longFrame)[sapply(1:ncol(longFrame), function(i) sum(is.na(longFrame[,i]))) > 0]
```
...
```{r cache=FALSE}
   ...
   features[[noSet[1]]] %>% nrow
```

### Combine Wide Sets:
<!--- : R code (No Results in Document) -->
```{r cache=FALSE}
   #fixlong = features[[2]] %>% gather(columnName,value,-orderID) %>% filter(!is.na(value)) %>% mutate(varName = gsub("ratio[0-9]","ratio",columnName)) %>% mutate(couponCol=1*grepl("ratio1",columnName) + 2*grepl("ratio2",columnName) + 3*grepl("ratio3",columnName)) %>% select(-columnName) %>% spread(varName,value) %>% arrange(orderID,couponCol)
...
```{r cache=FALSE}
   ...
   longFrame = longFrame %>% left_join(features[[2]],by=c("orderID","couponCol"))
```


### Check Set 1 Features
We can check the final results:
<!--- : R code (No Results in Document) -->
```{r cache=FALSE}
   dim(wideFrame)
   ...
   names(wideFrame)[colSums(is.na(wideFrame)) > 0]
   names(longFrame)[colSums(is.na(longFrame)) > 0]
```
...
<!--- : R code (No Results in Document) -->
```{r cache=FALSE}
   features[[2]] = features[[2]] %>% gather(columnName,value,-orderID) %>% filter(!is.na(value)) %>% mutate(varName = gsub("ratio[0-9]","ratio",columnName)) %>% mutate(couponCol=1*grepl("ratio1",columnName) + 2*grepl("ratio2",columnName) + 3*grepl("ratio3",columnName))  %>% select(-columnName) %>% spread(varName,value) %>% arrange(orderID,couponCol)
   wideFrame = features[[wideSet[1]]] %>% select(orderID)
   for(x in wideSet){
      dsn.i = features[[x]]
      dropCols = which(names(dsn.i) %in% names(wideFrame) & names(dsn.i) != "orderID")
