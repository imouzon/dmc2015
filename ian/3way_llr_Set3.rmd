...
```{r set-parent, echo=FALSE,warning=FALSE,error=FALSE,message=FALSE,cache=TRUE,warning=FALSE,include=FALSE,comment=NA}
   ...
   if(FALSE) 
      rmarkdown::render("/Users/user/dmc2015/ian/3way_llr_Set3.rmd")
```
...
```{r cache=FALSE}
   ## two way
   twoway = combn(c(groupB,groupD1,groupD2),2) %>% cbind(combn(c(groupC2,groupD1),2))
...
```{r cache=FALSE}
   ## three way 
   threeway = combn(c(groupB),3) %>%                                                             
      cbind(do.call("cbind", lapply(groupB, function(x) combn(c(x,groupD1,groupD2),3)))) %>%    
      cbind(do.call("cbind",lapply(c(groupD1,groupD2), function(x) combn(c(x,groupB),3)))) %>% 
