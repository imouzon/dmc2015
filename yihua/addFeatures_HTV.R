addFeatures_HTV <- function(H_melt, T_melt, V_melt){
  ...
  oneway = c("couponID", "couponsReceivedTime", "couponsReceivedDoW",
             ...
             "ratio_bp_p_round")
  temp = addKey_UniqueUser(H_melt, T_melt, V_melt, key=oneway[1])
  for(i in 2:length(oneway)){
    temp = addKey_UniqueUser(temp$H_melt, temp$T_melt, temp$V_melt, 
                             key=oneway[i])
  }
