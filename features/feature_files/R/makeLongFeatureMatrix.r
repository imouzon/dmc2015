#--------------------------------------**--------------------------------------#
#  File Name: makeLongFeatureMatrix.r
#  Purpose:
#
#  Creation Date: 14-05-2015
#  Last Modified: Thu May 14 19:34:21 2015
#  Created By:
#
#--------------------------------------**--------------------------------------#
#
...
makeLongFeatureMatrix = function(l,u,set){
   ...
   trn = list("X" = Xtrn %>% select(-orderID,-couponUsed,-basketValue),
              "y" = Xtrn %>% select(orderID,couponUsed, basketValue))

   #val
   Xval = HTV$V %>% select(orderID) %>% left_join(f1,by="orderID") %>% arrange(orderID)
   val = list("X" = Xval %>% select(-orderID,-couponUsed,-basketValue),
              "y" = Xval %>% select(orderID,  couponUsed, basketValue))

   #class
   Xcls = HTV$C %>% select(orderID) %>% left_join(f1,by="orderID") %>% arrange(orderID)
   cls = list("X" = Xcls %>% select(-orderID,-couponUsed,-basketValue),
              "y" = Xcls %>% select(orderID,couponUsed, basketValue))

   Fmat1 = list("train" = trn, "class" = cls, "validation" = val)
   saveRDS(Fmat1,file=paste0("~/dmc2015/data/featureMatrix/featMat_based-on-HTV",set,"_LONG_ver0.rds"))
}
