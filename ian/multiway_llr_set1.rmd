...
<!--- : R code (No Results in Document) -->
```{r cache=FALSE}
   qplot(couponsReceived,orderTime,color=basketGroup,data=d) + facet_wrap(~basketGroup)
```
...
<!--- : R code (No Results in Document) -->
```{r cache=FALSE}
pi1 = (sum(HTVset1$H$coupon1Used) + alpha.est)/(nrow(HTVset1$H) + alpha.est + beta.est)
pi2 = (sum(HTVset1$H$coupon2Used) + alpha.est)/(nrow(HTVset1$H) + alpha.est + beta.est)
pi3 = (sum(HTVset1$H$coupon3Used) + alpha.est)/(nrow(HTVset1$H) + alpha.est + beta.est)
```
