...
%  Last Modified: Sun Jun 21 17:33:38 2015
%  Created By:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%-- Set parent file
...
<<batches, echo=TRUE, cache=FALSE, include = TRUE>>=
   GetBatchInfo = function(initial_batch.ymd_hms, nbatch=10, weeks2expire = 1, train=trn, test=tst){
      ...
      p2 = ggplot() +geom_rect(data=couponBatches, aes(xmin=sendDate, xmax=sendDate+weeks(1), ymin=sendDate, ymax=expireDate, fill = batch),size=I(.4)) + geom_point(data=cbind(rbind(train,test),batch.invalid),aes(x=couponsReceived,y=orderTime),size=I(.4)) + coord_fixed(ratio=1)
...
<<assign_batch, echo=TRUE, cache=FALSE, include = TRUE>>=
   ...
   pdf("~/dmc2015/presentation/team_1_pres/www/figs/chenplot2.pdf",height=5,width=5)
      batchres$plots[[2]]
   dev.off()
@
%$
