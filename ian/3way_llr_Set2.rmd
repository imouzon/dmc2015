...
```{r set-parent, echo=FALSE,warning=FALSE,error=FALSE,message=FALSE,cache=TRUE,warning=FALSE,include=FALSE,comment=NA}
   ...
   if(FALSE) 
      rmarkdown::render("/Users/user/dmc2015/ian/3way_llr_Set2.rmd")
```
...
```{r splitcols,cache=FALSE,echo=FALSE}
...
```{r readHist1,cache=FALSE}
   HTVset2 = readRDS("~/dmc2015/data/featureMatrix/HTVset2.rds")
...
```{r cache=FALSE}
   ...
   result = llr_multiway(dm,HTVset2$H,colsel)
...
CreateSets = function(varns){
   ...
   #save long results
   result$long %>% saveRDS(file = paste0("~/dmc2015/features/feature_files/set2/llr_",filename,"_long.rds"))

   #save wide results
   result$wide %>% saveRDS(file = paste0("~/dmc2015/features/feature_files/set2/llr_",filename,"_wide.rds"))
}
```
...
```{r cache=FALSE}
# one way
oneway = c(groupA,groupB,groupC2,groupD1,groupD2)
...
```{r cache=FALSE}
## two way
twoway = combn(c(groupB,groupD1,groupD2),2) %>% 
...
```{r cache=FALSE}
## three way 
threeway = combn(c(groupB),3) %>%                                                              # group B = 2 and userID is OK   
   ...
   cbind(combn(c(groupD1,groupD2),3))
