...
```{r cache=FALSE}
   ...
   #sim_lnorm_pif
   cos.d4 = d %>% select(orderID) %>% cbind(t(sapply(1:nrow(dmcsucks), function(i) CosSimSummary(i,sim_lnorm_pif)))) %>% data.frame %>% renm(c("orderID", "sim_lnorm_pif_cosSim12", "sim_lnorm_pif_cosSim13", "sim_lnorm_pif_cos.sim23"))
...
```{r cache=FALSE}
   ...
   simFeatures = function(sim_mat){
      ...
      p %>% ggplot(aes(fill=usedA,x=similarity,alpha=.2)) + facet_grid(usedB~.) + geom_density() %>% print

      p %>% ggplot(aes(fill=interaction(usedA,usedB),x=fill,y=log(similarity),alpha=.2)) + geom_boxplot() %>% print
   }
   
```
<!--- chunk-label: R code (No Results in Document) -->
```{r fig.width=8,fig.height=5,out.width='.8\\linewidth',out.height='.5\\linewidth'}
