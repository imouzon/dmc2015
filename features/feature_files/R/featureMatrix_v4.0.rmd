...
```{r read_files_into_R,cache=FALSE}
   chk = readRDS("../set1/llr_cpnXordDoW.rds")
...
dim(chk)
   ...
   HTV_list = HTV1
   createFeatureMatrix = function(HTV_list,files,keepHTVcolumns,response_columns){
      if(!is.numeric(keepHTVcolumns)) keepHTVcolumns = which(names(HTV_list$T) %in% c("orderID",keepHTVcolumns))
      if(!is.numeric(response_cols)) response_cols = which(names(HTV_list$T) %in% keepHTVcolumns)

      trn = HTV_list$T[,keepHTVcolumns]
      val = HTV_list$V[,keepHTVcolumns]
      cls = HTV_list$C[,keepHTVcolumns]

      i = 1
      for(i in 1:length(files)){
         feature = readRDS(files[i])
         if(is.list(feature)){
            if("couponCol" %in% names(feature$T)){


            trn = trn %>% left_join(
                                    ,by="orderID")
            val = val %>% left_join(feature$V,by="orderID")
            cls = cls %>% left_join(feature$C,by="orderID")
         }else{
            trn = trn %>% left_join(feature$T,by="orderID")
            val = val %>% left_join(feature$V,by="orderID")
            cls = cls %>% left_join(feature$C,by="orderID")
         }
         str(feature)
