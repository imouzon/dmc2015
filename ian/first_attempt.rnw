...
%  Last Modified: Wed Apr  8 19:11:54 2015
%  Created By:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%-- Set parent file
...
<<paks, echo=TRUE, cache=TRUE, message=FALSE, include=TRUE>>=
   library(ggplot2)
   library(lubridate)
   library(xtable)
   library(rCharts)
...
<<scanData, echo=TRUE, cache=TRUE, include = TRUE>>=
...
<<readDat, echo=TRUE, cache=TRUE, include=TRUE>>=
...
<<sessioninfo, echo=TRUE, cache=TRUE, include=TRUE, results='asis'>>=
...
Coupon 1:
<<coupon1info, echo=TRUE, cache=TRUE, include=TRUE, results='asis'>>=
...
Coupon 2:
<<coupon2info, echo=TRUE, cache=TRUE, include=TRUE, results='asis'>>=
...
Coupon 3:
<<coupon3info, echo=TRUE, cache=TRUE, include=TRUE, results='asis'>>=
...
<<manageTime, echo=TRUE, cache=TRUE, include = TRUE>>=
...
<<nnss, echo=TRUE, cache=TRUE, include = TRUE>>=
   #longest time to act on coupon
   as.period(max(trn$orderTime - trn$couponsReceived))

   #shortest time to act on coupon
   as.period(min(trn$orderTime - trn$couponsReceived))
@
...
<<batches, echo=TRUE, cache=TRUE, include = TRUE>>=
...
<<assign_batch, echo=TRUE, cache=TRUE, include = TRUE>>=
   #this batch leads to bad results
   batchres = GetBatchInfo("2015-01-06 9:00:00")
@
...
<<gapsTuesday, echo=TRUE, cache=TRUE, include = TRUE>>=
   #combine the results
   d = rbind(trn,tst)
   ...
   batch.timing = d[which(d$orderTimeDoW == "Tuesday" & d$couponsReceivedDoW == "Tuesday"),]

   #we also know that 9:00 is not so great, so we can remove customers
   #who made purcahses after 9:00 am
   batch.timing = batch.timing[which(duration(interval(batch.timing$couponsReceived, batch.timing$orderTime)) < 60*60*9),]

   #lets look for gaps
   b.m = melt(batch.timing,c("orderID","couponsReceivedDate","orderTimeDate"),c("orderTime","couponsReceived"))

   qplot(value,couponsReceivedDate,data=b.m,geom='line',group=orderID) + facet_grid(.~couponsReceivedDate,scales='free_x')
@
