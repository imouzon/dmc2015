...
```{r dgg,cache=FALSE}
   ...
   fml3 = readRDS("../data/featureMatrix/featMat_based-on-HTVset1_LONG_ver0.3.rds")
   fml3$train$X = fml$train$X %>% left_join(fml3$train$X[,names(fml3$train$X)[which(!(names(fml3$train$X) %in% names(fml$train$X)) | names(fml3$train$X) %in% names(fml$train$X))]],by=c("orderID","couponCol"))
   fml3$validation$X = fml$validation$X %>% left_join(fml3$validation$X[,names(fml3$validation$X)[which(!(names(fml3$validation$X) %in% names(fml$validation$X)) | names(fml3$validation$X) %in% names(fml$validation$X))]],by=c("orderID","couponCol"))
   fml3$class$X = fml$class$X %>% left_join(fml3$class$X[,names(fml3$class$X)[which(!(names(fml3$class$X) %in% names(fml$class$X)) | names(fml3$class$X) %in% names(fml$class$X))]],by=c("orderID","couponCol"))

   saveRDS(fml3,"~/dmc2015/data/featureMatrix/featMat_based-on-HTVset1_LONG_ver0.6.rds")

   ## wide
   fmw = readRDS("../data/featureMatrix/featMat_based-on-HTVset1_WIDE_ver0.5.rds")
   fmw3 = readRDS("../data/featureMatrix/featMat_based-on-HTVset1_WIDE_ver0.3.rds")
   fmw3$train$X = fmw$train$X %>% left_join(fmw3$train$X[,names(fmw3$train$X)[which(!(names(fmw3$train$X) %in% names(fmw$train$X)) | names(fmw3$train$X) %in% names(fmw$train$X))]],by=c("orderID"))
   fmw3$validation$X = fmw$validation$X %>% left_join(fmw3$validation$X[,names(fmw3$validation$X)[which(!(names(fmw3$validation$X) %in% names(fmw$validation$X)) | names(fmw3$validation$X) %in% names(fmw$validation$X))]],by=c("orderID"))
   fmw3$class$X = fmw$class$X %>% left_join(fmw3$class$X[,names(fmw3$class$X)[which(!(names(fmw3$class$X) %in% names(fmw$class$X)) | names(fmw3$class$X) %in% names(fmw$class$X))]],by=c("orderID"))

   #estimate weights from the historical data:
   HTVset = readRDS("~/dmc2015/data/featureMatrix/HTVset1.rds")
   saveRDS(fmw3,"~/dmc2015/data/featureMatrix/featMat_based-on-HTVset1_WIDE_ver0.6.rds")
