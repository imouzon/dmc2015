...
```{r set-parent,echo=FALSE,warning=FALSE,cache=FALSE,include=FALSE,comment=NA}
   ...
   if(FALSE) 
      rmarkdown::render("featureMatrix_v3.rmd")
```
...
```{r chunk-label,cache=FALSE,eval=FALSE}
...
```{r batchIN,cache=FALSE}
   ...
   batchInfo_train = readRDS("~/dmc2015/features/feature_files/batchInfo_train.rds")
   batchInfo_class = readRDS("~/dmc2015/features/feature_files/batchInfo_test.rds")
   batchInfo = rbind(batchInfo_train,batchInfo_class)
```
...
```{r btchfromt,cache=FALSE}
  head(batchInfo$couponsReceivedTime)
as.numeric(head(batchInfo$couponsReceivedTime))
```
...
```{r checkftt,cache=FALSE}
   ...
   nrow(batchInfo_class) - length(complete.cases(batchInfo_class))
   nrow(batchInfo_train) - length(complete.cases(batchInfo_train))
```
