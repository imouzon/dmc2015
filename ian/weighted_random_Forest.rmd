...
```{r cache=FALSE}
   ...
   message("VALIDATION SET")
      ...
      uw.fitted.loss = colSums((matrix(yv,ncol=3,byrow=TRUE) - matrix(uw.fitted,ncol=3,byrow=TRUE))^2)
      uw.scaled.fitted.loss = colSums((matrix(yv,ncol=3,byrow=TRUE) - matrix(unweight.fit,ncol=3,byrow=TRUE))^2)/colMeans((matrix(yv,ncol=3,byrow=TRUE))^2)

      #Weighted Loss Aspects
      w.fitted = predict(weighted.rf,newdata = Xv,type="response")*rep(c(p1,p2,p3),times=nrow(Xv)/3)
      w.fitted.loss = colSums((matrix(yv,ncol=3,byrow=TRUE) - matrix(w.fitted,ncol=3,byrow=TRUE))^2)
      w.scaled.fitted.loss = colSums((matrix(yv,ncol=3,byrow=TRUE) - matrix(unweight.fit,ncol=3,byrow=TRUE))^2)/colMeans((matrix(yv,ncol=3,byrow=TRUE))^2)
