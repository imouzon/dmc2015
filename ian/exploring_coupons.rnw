...
%  Last Modified: Wed Apr 22 18:21:12 2015
%  Created By:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%-- Set parent fil
...
<<assign_batch, echo=TRUE, cache=FALSE, include = TRUE>>=
   ...
   trn.w = trn[,c("orderID",names(trn)[!(names(trn) %in% names(trn.raw))])]
   tst.w = tst[,c("orderID",names(tst)[!(names(tst) %in% names(tst.raw))])]
   head(trn.w)
@
