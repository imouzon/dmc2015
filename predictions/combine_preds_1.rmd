...
```{r cache=FALSE}
   ...
   #drop errors
   qplot(method_name[-which(validation_error > 16400)], validation_error[-which(validation_error > 16400)])

   files[method_name[-which(validation_error > 16400)]]
   ...
   plot_func(32,8) + ylim(c(15800,16400))
   names(validation_predictions)[c(8,32)]
   order(rowSums(cor(validation_predictions)[c(rep(TRUE,3),validation_error < 16400),c(32,8)]))
   cor(validation_predictions)[28,c(32,8)]
   names(validation_predictions)[28]

   #
   newpred1 = .5*validation_predictions[,8] + .5*validation_predictions[,32]
   clspred1 = .5*classification_predictions[,8] + .5*classification_predictions[,32]
   write.csv(matrix(clspred1,ncol=3,byrow=TRUE),"./predictions_based_on_1.csv")
   write.csv(matrix(clspred3,ncol=3,byrow=TRUE),"./predictions_based_on_3.csv")
   ...
   lossFunction(newpred,validation_predictions[,3])/length(newpred)
