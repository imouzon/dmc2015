...
```{r dgg,cache=FALSE}
   ## long 
   fml = readRDS("../data/featureMatrix/featMat_based-on-HTVset1_LONG_ver0.4.rds")

   ## wide
   fmw = readRDS("../data/featureMatrix/featMat_based-on-HTVset1_WIDE_ver0.4.rds")
...
```{r rf,cache=FALSE}
   ...
   chk = fmw$validation$y %>% summarize(coupon1 = mean(coupon1Used), coupon2 = mean(coupon2Used), coupon3 = mean(coupon3Used)) %>% gather(coupon,proportion)

   boots.melt %>% ggplot(aes(x=proportion,fill=coupon)) + geom_density() + facet_grid(~coupon) + geom_vline(data=chk,aes(xintercept = proportion, color=coupon))
...
```{r cache=FALSE}
   ...
   sim_columns = names(Xn)[c(3,grep("sim_", names(Xn)))]
   sim_columns = sim_columns[c(1,2,3,10,14,16,17,18)]

   ## get single way llrs:
   llr1_columns = names(Xn)[which(grepl("llr", names(Xn)) & !grepl("X",names(Xn)))]

   #EarlyRec
   coupon = names(Xn)[grepl("llr",names(Xn)) & grepl("cpn",names(Xn))]

   #zprice orig
   zprice_orig = names(Xn)[grepl("orig",names(Xn))]

   #The predictor and response columns
   Xrf = Xn[,unique(c(sim_columns,llr1_columns,zprice_orig,coupon))]
...
```{r chunk-label,cache=FALSE}
   ...
   %in% names(Xv)
   predict(weighted.rf,newdata = Xv,type="response")
   *rep(c(p1,p2,p3),times=nrow(Xv)/3)
