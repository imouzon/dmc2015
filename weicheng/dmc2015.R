...
dat = within(dat, {
    ...
    levels(userID) = paste0("u", 1:nlevels(userID))
    couponsReceived = as.POSIXct(couponsReceived)
    levels(couponID1) = paste0("cou1_", 1:nlevels(couponID1))
    levels(couponID2) = paste0("cou2_", 1:nlevels(couponID2))
    levels(couponID3) = paste0("cou3_", 1:nlevels(couponID3))
    levels(brand1) = paste0("bra1_", 1:nlevels(brand1))
    levels(brand2) = paste0("bra2_", 1:nlevels(brand2))
    levels(brand3) = paste0("bra3_", 1:nlevels(brand3))
    levels(categoryIDs1) = paste0("cat1_", 1:nlevels(categoryIDs1))
    levels(categoryIDs2) = paste0("cat2_", 1:nlevels(categoryIDs2))
    levels(categoryIDs3) = paste0("cat3_", 1:nlevels(categoryIDs3))
    levels(productGroup1) = paste0("gr1_", 1:nlevels(productGroup1))
    levels(productGroup2) = paste0("gr2_", 1:nlevels(productGroup2))
    levels(productGroup3) = paste0("gr3_", 1:nlevels(productGroup3))
})
...
sum(!is.na(dat$basketValue))
sum(!is.na(dat$coupon1Used))
sum(!is.na(dat$coupon2Used))
sum(!is.na(dat$coupon3Used))

dat$orderID
head(dat$couponsReceived)
head(dat$couponID1)
head(dat$price1)
head(dat$)
...
oT = as.POSIXct(dat$orderTime)
crT = as.POSIXct(dat$couponsReceived)
## hist in minutes
hist(as.numeric(difftime(oT, crT, units="mins")), 30)
## barplot in days
barplot(table(as.integer(difftime(oT, crT, units="days"))))
