...
<!--- : R code (No Results in Document) -->
```{r cache=FALSE}
   fmat = readRDS("~/dmc2015/data/featureMatrix/featMat_based-on-HTVset3_WIDE_ver0.3.rds")
   HTV = readRDS("~/dmc2015/data/featureMatrix/HTVset3.rds")
   ...
   #new features
   files = list.files("../set3/new_llrs",full.names=TRUE)
```
...
```{r cache=FALSE}
   #previous features
   fml = readRDS("~/dmc2015/data/featureMatrix/featMat_based-on-HTVset3_LONG_ver0.3.rds")
   class_add = readRDS("../../../yihua/HTVCmelt3_Combn_UniqueUser.rds")
   names(fml)
...
<!--- chunk-label: R code (No Results in Document) -->
```{r cache=FALSE}
   fmw = readRDS("~/dmc2015/data/featureMatrix/featMat_based-on-HTVset3_WIDE_ver0.3.rds")
   wide.files = files[grep("wide",files)]
   featureswide = lapply(wide.files, function(x) readRDS(x))
   ...
   fmw$class$X %>% gather(colname,colval) %>% mutate(colval = is.na(colval)) %>% group_by(colname) %>% summarize(freqna = sum(colval)) %>% filter(freqna > 0) %>% arrange(-freqna)
```

## save them
```{r cache=FALSE}
   saveRDS(fml,"~/dmc2015/data/featureMatrix/featMat_based-on-HTVset3_LONG_ver0.4.rds")

   saveRDS(fmw,"~/dmc2015/data/featureMatrix/featMat_based-on-HTVset3_WIDE_ver0.4.rds")
```
