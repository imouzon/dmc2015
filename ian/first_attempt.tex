...
%  Last Modified: Wed Apr  8 19:11:28 2015
%  Created By:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%-- Set parent file
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): could not find function "{}qplot"{}}}\end{kframe}
\end{knitrout}
...
\hlkwd{as.period}\hlstd{(}\hlkwd{max}\hlstd{(trn}\hlopt{$}\hlstd{orderTime} \hlopt{-} \hlstd{trn}\hlopt{$}\hlstd{couponsReceived))}
\end{alltt}


{\ttfamily\noindent\itshape\color{messagecolor}{\#\# estimate only: convert difftimes to intervals for accuracy}}\begin{verbatim}
## [1] "6d 9H 37M 44S"
\end{verbatim}
\begin{alltt}
\hlcom{# shortest time to act on coupon}
\hlkwd{as.period}\hlstd{(}\hlkwd{min}\hlstd{(trn}\hlopt{$}\hlstd{orderTime} \hlopt{-} \hlstd{trn}\hlopt{$}\hlstd{couponsReceived))}
\end{alltt}
...
\hlstd{batchres} \hlkwb{<-} \hlkwd{GetBatchInfo}\hlstd{(}\hlstr{"2015-01-06 9:00:00"}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
Customers are using coupons much more than a week after they were sent out if 
the first batch was sent at 9:00 am:
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): could not find function "{}weeks"{}}}\begin{alltt}
...
\hlcom{# on a Tuesday}
\hlstd{batch.timing} \hlkwb{<-} \hlstd{d[}\hlkwd{which}\hlstd{(d}\hlopt{$}\hlstd{orderTimeDoW} \hlopt{==} \hlstr{"Tuesday"} \hlopt{&}
    \hlstd{d}\hlopt{$}\hlstd{couponsReceivedDoW} \hlopt{==} \hlstr{"Tuesday"}\hlstd{), ]}

\hlcom{# we also know that 9:00 is not so great, so we can}
\hlcom{# remove customers who made purcahses after 9:00 am}
\hlstd{batch.timing} \hlkwb{<-} \hlstd{batch.timing[}\hlkwd{which}\hlstd{(}\hlkwd{duration}\hlstd{(}\hlkwd{interval}\hlstd{(batch.timing}\hlopt{$}\hlstd{couponsReceived,}
    \hlstd{batch.timing}\hlopt{$}\hlstd{orderTime))} \hlopt{<} \hlnum{60} \hlopt{*} \hlnum{60} \hlopt{*} \hlnum{9}\hlstd{), ]}

\hlcom{# lets look for gaps}
\hlstd{b.m} \hlkwb{<-} \hlkwd{melt}\hlstd{(batch.timing,} \hlkwd{c}\hlstd{(}\hlstr{"orderID"}\hlstd{,} \hlstr{"couponsReceivedDate"}\hlstd{,}
    \hlstr{"orderTimeDate"}\hlstd{),} \hlkwd{c}\hlstd{(}\hlstr{"orderTime"}\hlstd{,} \hlstr{"couponsReceived"}\hlstd{))}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): could not find function "{}melt"{}}}\begin{alltt}
\hlkwd{qplot}\hlstd{(value, couponsReceivedDate,} \hlkwc{data} \hlstd{= b.m,} \hlkwc{geom} \hlstd{=} \hlstr{"line"}\hlstd{,}
    \hlkwc{group} \hlstd{= orderID)} \hlopt{+} \hlkwd{facet_grid}\hlstd{(.} \hlopt{~} \hlstd{couponsReceivedDate,}
    \hlkwc{scales} \hlstd{=} \hlstr{"free_x"}\hlstd{)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in ggplot(data, aesthetics, environment = env): object 'b.m' not found}}\end{kframe}
\end{knitrout}
